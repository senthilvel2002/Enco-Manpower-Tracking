  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(apiUrl("/api/auth/admin-login"), {
        ...
        body: JSON.stringify({ civil_id: civilId, view_type: "site_incharge", password }),
      });
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10 sm:py-12 pb-[max(2.5rem,env(safe-area-inset-bottom,0px))]">
        <div className="w-full max-w-md">
          <div className="rounded-2xl border border-white/10 bg-slate-900/70 backdrop-blur-xl p-8 shadow-2xl">
            ...
            <h1 className="text-2xl font-black text-white text-center mb-1">Site Incharge Login</h1>
            <p className="text-slate-400 text-sm text-center mb-8">
              Use the last 4 digits of your Civil ID or your full ID, plus your password
            </p>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="siteInchargeCivilId" className={labelCls}>Civil ID (last 4 or full)</label>
                <input
                  id="siteInchargeCivilId"
                  type="text"
                  className={inputCls}
              ...
              <div>
                <label htmlFor="siteInchargePassword" className={labelCls}>Password</label>
                <input
                  id="siteInchargePassword"
                  type="password"
                  className={inputCls}
              ...
              <button
                type="submit"
                className="w-full rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-sm font-bold py-3 transition-all duration-200 hover:shadow-[0_0_24px_rgba(245,158,11,0.25)] mt-2"
              >
                Sign In
              </button>
            </form>
...
export default SiteInchargeLogin;
