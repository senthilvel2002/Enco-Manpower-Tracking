...
const inputCls =
  "w-full rounded-xl border border-white/15 bg-slate-900/80 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:border-amber-400/60 focus:outline-none focus:ring-2 focus:ring-amber-400/20 transition";
const labelCls = "block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5";

function EquipmentLogin() {
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10 sm:py-12 pb-[max(2.5rem,env(safe-area-inset-bottom,0px))]">
        <div className="w-full max-w-md">
          <div className="rounded-2xl border border-white/10 bg-slate-900/70 backdrop-blur-xl p-8 shadow-2xl">
            <div className="w-14 h-14 rounded-2xl bg-amber-500/15 border border-amber-400/30 flex items-center justify-center text-3xl mb-6 mx-auto">
              🚜
            </div>
            <h1 className="text-2xl font-black text-white text-center mb-1">Equipment Login</h1>
            <p className="text-slate-400 text-sm text-center mb-8">
              Last 4 digits of your Civil ID, or full ID — then log equipment entry
            </p>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="equipmentCivilId" className={labelCls}>Civil ID (last 4 or full)</label>
                <input
                  id="equipmentCivilId"
                  type="text"
                  className={inputCls}
              ...
              <button
                type="submit"
                className="w-full rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-sm font-bold py-3 transition-all duration-200 hover:shadow-[0_0_24px_rgba(251,191,36,0.25)] mt-2"
              >
                Sign In
              </button>
            </form>
            ...
            <div className="mt-6 text-center">
              <Link to="/" className="text-slate-400 hover:text-amber-300 text-sm transition no-underline">
                ← Back to Home
              </Link>
            </div>
          </div>
          ...
          <p className="text-center text-xs text-slate-600 mt-6">
            Encogroup · Sohar Hub Program · Equipment Tracking
          </p>
        </div>
      </main>
    </div>
